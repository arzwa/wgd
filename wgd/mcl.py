...
def family_stats(families_file, pro_file=None):
    """
    Generate a phylogenetic profile and some stats
    ...
    with open(families_file, 'r') as f:
        for line in f:
            ...
            family_id = 'F{0:0>5}'.format(family)
            gene_families_dict[family_id] = {}

            for gene in line:
                sp, gene_id = gene.split('|')

                if sp not in gene_families_dict[family_id]:
                    gene_families_dict[family_id][sp] = 0
                gene_families_dict[family_id][sp] += 1

            family += 1

    profile = pd.DataFrame.from_dict(gene_families_dict, orient='index').fillna(0)
    row_sum = profile.sum(axis=1)

    summary = {'Genes': {}, 'Singletons': {}, 'Species-specific multicopy': {}}
    for sp in profile.columns:
        summary['Genes'][sp] = profile.sum()[sp]
        summary['Singletons'][sp] = len(profile[profile[sp] == 1])
        summary['Species-specific multicopy'][sp] = len(profile[profile[sp] == row_sum])

    print('{0:>20}:{1:>10}'.format('Number of families', len(profile.index)))
    print('{0:>20}:{1:>10}'.format('Single-copy core', len(row_sum[row_sum == len(profile.columns)])))
    print()
    print(pd.DataFrame.from_dict(summary, orient='index'))

    if pro_file:
        profile.to_csv(pro_file, sep='\t', index_col=0)
